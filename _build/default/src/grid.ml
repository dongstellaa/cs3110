...
let left_shift row =
  match List.length (delete_zeros row) with
  | 0 -> delete_zeros row @ [ 0; 0; 0; 0 ]
  | 1 -> delete_zeros row @ [ 0; 0; 0 ]
  | 2 -> delete_zeros row @ [ 0; 0 ]
  | 3 -> delete_zeros row @ [ 0 ]
  | _ -> delete_zeros row @ []
(* let right_shift row = List.rev row |> shift_left |> List.rev *)

let right_shift row =
  match List.length (delete_zeros row) with
  | 0 -> [ 0; 0; 0; 0 ] @ delete_zeros row
  | 1 -> [ 0; 0; 0 ] @ delete_zeros row
  | 2 -> [ 0; 0 ] @ delete_zeros row
